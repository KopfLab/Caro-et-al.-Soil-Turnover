...
results_trip <- results %>% 
  ...
  # Replace NA Height and Area values with Zero in order to do summarizing operations
  mutate(`Relative Height` = replace(`Relative Height`, is.na(`Relative Height`), 0)) %>%
  mutate(`Relative Area` = replace(`Relative Area`, is.na(`Relative Area`), 0)) %>% 
  # Generate summary stats
  summarize(rel_area_max = max(`Relative Area`),
            rel_area_min = min(`Relative Area`),
            rel_height_max = max(`Relative Height`),
            rel_height_min = min(`Relative Height`),
            `Retention Time` = mean(`Retention Time`),
            `Relative Area` = mean(`Relative Area`),
            `Relative Height` = mean(`Relative Height`)) %>% 
