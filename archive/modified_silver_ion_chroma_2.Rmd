...
results <- results %>% 
  mutate(saturation = case_when(
    str_detect(`Peak Name`, "C19 monoene") ~ "C19:1",
...
p_sfames <- plot_results %>%
  ggplot() +
  aes(`Retention Time`, `Relative Height`, fill = `Peak Name`, key = `Peak Name`, label = saturation) +
  geom_bar(stat = 'identity', width = 0.1) +
  facet_wrap(~fraction_eluent_f, ncol = 1, scales = "free_y") +
  ggtitle("Boulder Soil Free FA as FAMES") +
  geom_label_repel(data = subset(plot_results, saturation %in% c("n1 cis", "n1 trans")),
                   aes(vjust =-7, box.padding = 0.5),
                   show.legend = FALSE,
                   size = 2,
                   segment.alpha = 0.5) +
  theme_bw() +
