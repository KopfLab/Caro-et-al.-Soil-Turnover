...
5.  Elute **F1** with 4mL hexane. *F1 should rule out solvent carry-through.*
...
2.  Divide in half: Use 100µL as a **standard**. Use 100µL as a **sample.** *The column sees 100µL \* 10ng/µL = 1000ng (1µg) per compound).*
...
plot_results <- plot_results %>% 
  mutate(fraction_eluent = case_when(
    sample == "tcbames3" ~ "NA",
    sample == "BAME_modagion_F1" ~ "F1 Hexane",
    sample == "BAME_modagion_F2" ~ "F2 99:1 Hex:ac",
    sample == "BAME_modagion_F3" ~ "F3 96:4 Hex:ac",
    sample == "BAME_modagion_F4" ~ "F4 90:10 Hex:ac",
    sample == "BAME_modagion_F5" ~ "F5 Acetone"
     )
  )

factored_eluents <- c('NA', 'F1 Hexane','F2 99:1 Hex:ac','F3 96:4 Hex:ac','F4 90:10 Hex:ac', 'F5 Acetone')
...
p_tc_bames2 <- plot_results %>%
  ...
  ggtitle("BAMES Standard 10ng/µL") +
  theme_bw()

ggsave(filename = "fig_output/p_tc_bames2.png", plot = p_tc_bames2, height = 10, width = 7)
