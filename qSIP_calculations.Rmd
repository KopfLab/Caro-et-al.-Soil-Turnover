...
FID_results_trip <- FID_results %>% 
  ...
  summarize(
    rel_area_FID_max = max(rel_area_FID, na.rm=TRUE),
...
## Select only samples that are included in analysis

```{r}
# Remove meadow samples and Ag-ion fractions
FID_results_abundance_reported <- FID_results_abundance_no_stds %>% 
  filter(
    str_detect(sample_id, "m0", negate = TRUE), # remove meadow samples
    str_detect(sample_id, "m3", negate = TRUE), # remove meadow 30cm samples
    str_detect(sample_id, "t0t", negate = TRUE) # remove amended tundra samples
  )

# Uncomment to view included samples:
# FID_results_abundance_reported %>% select(sample_id) %>% unique() %>% view()
```
