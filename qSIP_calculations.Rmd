...
> 1. Change script to calculate µ for individual replicates *then* calculate means of µ, not 
> calculate mean enrichment --> µ. (DONE).

> 2. Get standard error of F_L by taking se of each terrain F_L measurement (DONE).
--> No error estimate on the measurement itself, took se of measurements across each terrain.

> 3. Get se of F_0 by taking se of 21 and 23 PC standards at timepoint zero. (DONE)
--> subtract means from both measurements before taking se

4. Add additional calculation of µ between 0 and 3 days (DONE) and 3 and 7 days (in progress).

5. Get standard error of assimilation efficiency (`a`) by using literature epsilon values Seb has collected.
Question: Unclear how to get `a` from epsilon. (I should know this?)

6. Remake enrichment curves as ∆2F enrichment instead of 2F (in progress)
...
gen_calc_data <- peaks_mapped_with_zeros_analytes %>% 
  ...
  mutate(
    ...
    u_d = gencalc(a = a, # Assimilation efficiency
...
```{r}
gen_calc_data_3_to_7 <- peaks_mapped_with_threes_analytes %>% 
  mutate(
    # Explicitly define incubation parameters:
    f_label = F_label_mn, # label is adjusted for the effective 2F of the label in a specific terrain
    a = 0.5, # assimilation efficiency
    t0 = 0, # start time
    f_start = at2H_mn_three, # use time point three instead of timepoint zero
    d_t = inc_time_d - t0, # change in time
    f_t = `2F_alk`, # use PAME-corrected 2F value
...
```{r}
test_data <- peaks_mapped_with_zeros_analytes[249,]
test_data <- test_data %>% 
  mutate(
    su_d = calculate_sigma_mu(
      a = 1,
