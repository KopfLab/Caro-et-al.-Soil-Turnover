...
c_inc_time_zero <- inc_time_zero %>% 
  filter(str_detect(t_series_id, "c"))
         

c_peaks_mapped <- peaks_mapped %>% 
  filter(str_detect(t_series_id, "c")) %>% 
  full_join(c_inc_time_zero, by = c("t_series_id", "compound"))
...
peaks_mapped_with_zeros <- 
  ...
  left_join(
    zero_peaks %>%
      select(
        compound, t_series_id, zero_sample_id = sample_id, area2_zero = area2, d2H_zero = d2H
      ),
    by = c("t_series_id", "compound")
  ) 
