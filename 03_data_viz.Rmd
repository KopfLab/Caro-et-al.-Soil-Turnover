...
```{r}
 p_lit_turnover_w_ours_inset <- lit_turnover_w_ours %>%
  filter(method_type != "Microscopy") %>%
  ggplot(
    aes(
      x = u_d_mn, 
...
p_lit_turnover_w_ours_inset
```

### Draw with inset

```{r}
p_literature2_w_inset <- ggdraw(p_lit_turnover_w_ours2) +
  draw_plot(
    p_lit_turnover_w_ours_inset, 
    x = 0.63, 
    y = 0.23, 
...
p_turnover_totals <- turnover_weighted_w_totals %>% 
  ...
  labs(
    x = "B (µg)",
    y = TeX("µ $d^{-1}$"),
    caption = "Where B is total biomass measured at a given site, µg per g soil",
    color = ""
  ) +
  theme_bw() +
  theme(
...
```{r}
instrument_error <- mean(LH_SIP_data$F_0_at2H_se)
...
### IRMS instrument "turnover"

```{r}

instrument_turnover <- turnover_summarized %>% 
  mutate(
    instrument_u_d = calculate_turnover(
      a = a, # Assimilation efficiency defined in data reduction step
      FT = F_0_at2H_se, # 2F instrument error (at%)
      F0 = 0.0000000000000000000001, # 2F at t0 (at%)
      FL = f_label, # 2F of label (at%)
      t = d_t # change in time
      ),
    instrument_over_actual_u_d = instrument_u_d / u_d_mc
  ) %>%
  filter(inc_time_d == 7) %>% 
  group_by(compound, terrain) %>% 
  summarize(
    instrument_u_d = instrument_u_d,
    u_d_mc = u_d_mc,
    instrument_over_actual_u_d_mn = mean(
      instrument_over_actual_u_d, na.rm = TRUE
      )
    ) %>% 
  filter(
    !is.na(instrument_over_actual_u_d_mn),
    is.finite(instrument_over_actual_u_d_mn)
    ) %>% 
  filter(!compound %in% c("20:0", "22:0", "24:0", "18:0", "cyclo 17:0"))

instrument_frac_error <- mean(instrument_turnover$instrument_over_actual_u_d_mn)

paste0("The accuracy of IRMS is such that we can accurately quantify deuterium incorporation corresponding to ", round(instrument_frac_error *100, 2), "% of an organism's growth rate.")
...
turnover_summarized %>% 
  ...
  labs(
    x = "Growth Rate (day^-1)",
    y = "Memory Corrected Growth Rate (day^-1)"
  )
```
