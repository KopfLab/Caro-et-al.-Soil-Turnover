...
source(file.path("libs", "d2H_to_2F.R"))
...
# Turnover rates averaged across 3d, 7d, experimental replicates
turnover_summarized_avg <- readRDS("cache/turnover_summarized_avg.RDS")
...
turnover_weighted_avg <- readRDS("cache/turnover_weighted_avg")
...
# IRMS Error Estimation
sF_0 <- readRDS(file = "cache/sF_0.RDS")
...
lit_turnover_w_ours <- lit_turnover %>% 
  bind_rows(turnover_means) %>% 
...
p_dF <- turnover_summarized %>% 
  ...
  geom_point(alpha = 1) +
  geom_line() +
  facet_wrap(vars(terrain), scales = "free_y") +
  ...
  scale_size_discrete(range = c(0.25, 2)) +
...
plot_turnover <- function(df, terrain_choice, color_choice) {
  ...
  ggplot(
    ...
    theme_classic() +
    labs(
      x = TeX("Compound-specific growth rate (day$^{-1}$)"),
...
ggsave(
  plot = p_turnover,
  filename = "fig_output/p_turnover2.pdf",
...
ggsave(filename = "fig_output/p_dF_ud.pdf", 
       plot = p_dF_ud,
       device = cairo_pdf, # required for rendering unicode symbols
       width = 11,
       height = 8)
...
# Calculate summary statistics and export them
...
turnover_corr_test <- broom::tidy(
  cor.test(
    x = turnover_cleaned$compound_ug_per_g_soil,
    y = turnover_cleaned$u_d_mc,
    method = "pearson"
  )
  )
...
writexl::write_xlsx(turnover_corr_test, path = "data_output/turnover_corr_test.xlsx")
```

```{r}

# Plot it with the linear model!
p_abundance_v_turnover2 <- turnover %>% 
  ...
  scale_color_manual(values = terrain_color_swatch) +
  #scale_fill_manual(values = terrain_color_swatch) +
  #scale_x_log10() +
  #scale_y_log10() +
  #scale_alpha_continuous(range = c(0,0.5)) +
  theme_bw() +
  labs(
    x = TeX("PLFA Concentration (µg $\\cdot g^{-1}$ soil)"),
...
```{r}
total_biomass <- turnover_cleaned %>% 
  group_by(terrain) %>% 
  summarize(
    total_ug_per_g_soil = sum(compound_ug_per_g_soil, na.rm = TRUE)
  )

turnover_weighted_w_totals <- turnover_weighted %>% 
  filter(inc_time_d == 7) %>% 
  mutate(
    terrain = case_when(
      terrain == "Gordon Gulch Conifer Forest" ~ "Conifer Forest",
  ...
  left_join(total_biomass, by = "terrain")


b_ud_fit <- broom::tidy(
  lm(
    data = turnover_weighted_w_totals, 
    u_d_mc_weighted  ~ total_ug_per_g_soil
    )
  )
b_ud_stats <- broom::glance(
  lm(
    data = turnover_weighted_w_totals, 
    u_d_mc_weighted  ~ total_ug_per_g_soil
    )
...
p_turnover_totals <- turnover_weighted_w_totals %>% 
  ggplot(
    aes(
      x = total_ug_per_g_soil,
      y = u_d_mc_weighted
    )
  ) +
  geom_point(aes(color = terrain),
             size = 5) +
  # geom_smooth(
  #   method = "lm",
  #   formula = "y~x",
  ...
  # ) +
  scale_color_manual(values = terrain_color_swatch) +
  coord_cartesian(xlim = c(0, 3000), ylim = c(0.01, 0.05)) +
  labs(
    x = TeX("B (µg $\\cdot g^{-1}$ soil)"),
    y = TeX("Weighted mean \n growth rate (day$^{-1}$)"),
    color = ""
  ) +
  theme_bw() +
...
```{r}
saveRDS(turnover_weighted_w_totals, file = "cache/turnover_weighted_w_totals.RDS")
```
...
p_abundance_v_turnover_w_inset
...
p_lit_turnover_w_ours2 <- lit_turnover_w_ours %>% 
  mutate(
    method_type = fct_relevel(method_type, c("LH-SIP", "DNA-SIP", "TdR/Leu", "Nutrient Budgeting"))) %>% 
  arrange(method_type, desc(u_d_mn)) %>% 
  mutate(
    m = seq(1,nrow(lit_turnover_w_ours))
  ) %>% 
  ggplot(
  aes(
    x = -m,
  ...
  geom_col() +
  geom_text(aes(
    label = format(round(gen_d, 1)),
    color = method_type
    ),
    nudge_y = .01
    )+
  scale_fill_manual(values = lit_values_swatch_3) +
  scale_color_manual(values = lit_values_swatch_3) +
  ...
  labs(
    y = TeX("Apparent Growth Rate (day$^{-1}$)"),
    fill = "Method"
  ) +
  theme_classic() +
  theme(
...
p_lit_turnover_w_ours2

cowplot::save_plot(filename = "fig_output/literature_values_draft2.pdf",
                   plot = p_lit_turnover_w_ours2,
                   base_height = 8,
                   base_width = 10)
...
## SF: Memory effect on gen_d and u_d

```{r mean_generation_time}
p_gen_time_mean <- turnover_weighted %>% 
  pivot_longer(
    cols = c(gen_d_weighted, gen_d_mc_weighted),
    names_to = "Correction",
    values_to = "gen_d"
  ) %>% 
  mutate(terrain = case_when(
  ...
  scale_fill_manual(values = c("#4690b0", "#3d3d3d")) +
  labs(x = "Soil Type",
...
p_ud_mean <- turnover_weighted %>% 
  pivot_longer(
    cols = c(u_d_weighted, u_d_mc_weighted),
    names_to = "Correction",
    values_to = "u_d"
  ) %>% 
  mutate(terrain = case_when(
  ...
  labs(
    x = "Soil Type",
    y = TeX("Weighted Mean Growth Rate $(d^{-1})$")
    ) +
  theme_classic() +
  theme(
    legend.position = "NA",
...
p_memcorr_effect <- cowplot::plot_grid(
  p_gen_time_mean,
  p_ud_mean,
  nrow = 1
)
...
cowplot::save_plot(
  plot = p_memcorr_effect,
  filename = "fig_output/memory_correction.pdf",
  base_height = 2.5,
  base_width = 6
)
```
...
p_time_reqd_for_visible_colony <- colony_data_2 %>% 
  ggplot(aes(x = gen_time_d,
             y = time_reqd_d)) +
  geom_line() +
  geom_vline(xintercept = 44.9,
             color = tundra_color,
             linetype = "dotted") +
    geom_vline(xintercept = 19.3,
             color = grassland_color,
             linetype = "dotted") +
    geom_vline(xintercept = 14.1,
             color = conifer_color,
             linetype = "dotted") +
  annotation_logticks(sides = "l") +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x)
...
## SF: IRMS Error Assessment

```{r}
# Pull the IRMS standard error for this experiment from the error of measuring standards
IRMS_standard_error <- sF_0 %>% select(F_0_at2H_se) %>% pull()
paste0("The standard error of IRMS is +/- ", IRMS_standard_error, "% (", IRMS_standard_error*10000, " ppm)")


test_standard_error_u <- tibble(
  a = 1,
  FT = IRMS_standard_error, # standard error in atom%
  F0 = 0.00000000000000000001, # start at zero
  t = seq(1, 100, by = 1), # time ranging between 0 and 100 days
  FL = 100, # label strength in atom%
  u = calculate_turnover(
    a = a,
    FT = FT,
    F0 = F0,
    t = t,
...
test_standard_error_u %>% 
  ...
  labs(
    x = "Incubation Time",
    y = "Growth rate (day-1)",
    caption = paste0("Growth estimate resulting from", round(IRMS_standard_error*10000,4), " ppm F2H error across incubation times" )
  )
```

```{r}
error_testing <- turnover %>% 
  select(inc_time_d, dF, f_label, a, u_d_mc) %>% 
  mutate(F0 = 0) %>% 
  filter(
    inc_time_d != 0,
    !is.na(dF)) %>% 
  mutate(FT_se = IRMS_standard_error) %>% 
  mutate(
    u_d_FT_se = calculate_turnover(
      a = a, 
      FT = FT_se,
      F0 = F0,
      FL = f_label,
    ...
    turnover_ratio = u_d_FT_se / u_d_mc
  )
...
error_testing %>% 
  ggplot(
    aes(
      x = u_d_FT_se,
      fill = as.factor(inc_time_d),
      #shape = as.factor(f_label)
      )
  ) +
  geom_histogram() +
  labs(
    shape = "Label Strength (at%)",
    fill = "Incubation Time (Days)"
  )

error_testing_summary <- error_testing %>% 
  group_by(inc_time_d, f_label) %>% 
  summarize(
    u_d_FT_se_mn = mean(u_d_FT_se, na.rm = TRUE),
    FT_se_mn = mean(FT_se, na.rm = TRUE)
    ) %>% 
  rename(
    sensitivity_FT = FT_se_mn,
    sensitivity_u_d = u_d_FT_se_mn
    ) %>% 
  mutate(
    sensitivity_u_hr = sensitivity_u_d * 24,
    sensitivity_u_min = sensitivity_u_hr * 60,
    sensitivity_u_sec = sensitivity_u_min * 60
  ) %>% 
  select(inc_time_d, f_label, sensitivity_FT, sensitivity_u_d, sensitivity_u_hr, sensitivity_u_min, sensitivity_u_sec)

```

The resolution of LH-SIP is defined as the standard error of the IRMS instrument ($F_{SE}$) propagated through the turnover time calculation:

$$
\mu = - \frac{1}{t}ln(\frac{F_{SE} - a \cdot F_L}{F_0 - a \cdot F_L})
$$

Plugging in our standard error of:
```{r}
paste0("The standard error of IRMS is +/- ", IRMS_standard_error, "% (", IRMS_standard_error*10000, " ppm)")
```

```{r}
knitr::kable(error_testing_summary)
```




## SF: Sensitivity Testing
...
LH_SIP_data %>% 
  ...
  ggplot(
    aes(
      x = calibrated_at2H_without_memory_se,
      y = calibrated_at2H_with_memory_correction_se,
  ...
  labs(
    x = TeX("SE: Without Memory Correction ($^2$H %)"),
    y = TeX("SE: With Memory Correction ($^2$H %)")
  ) +
  theme_classic()
```

### IRMS instrument error

```{r}
instrument_error <- as.numeric(isotopia::to_abundance(
  isotopia::delta(
    `2H` = 8.9, major = "1H",
    ref = "VSMOW", notation = "permil")))
instrument_error_u_d <- calculate_turnover(
  a = 0.7,
  FL = 0.5,
  F0 = 0.00000000000000000000001,
  FT = instrument_error,
  t = 7
)


instrument_error_enriched <- as.numeric(isotopia::to_abundance(
  isotopia::delta(
    `2H` = 60, major = "1H",
    ref = "VSMOW", notation = "permil")))

instrument_error_enriched_u_d <- calculate_turnover(
  a = 0.7,
  FL = 0.5,
  F0 = 0.00000000000000000000001,
  FT = instrument_error_enriched,
  t = 7
)

library(isotopia)
tibble(
  d2H_error_permil = c(8.9, 60),
  at2H_error_percent = 100*as.numeric(
    to_abundance(delta(
    `2H` = d2H_error_permil, major = "1H",
    ref = "VSMOW", notation = "permil"))
    ),
  at2H_error_ppm = 10000*at2H_error_percent,
  growth_rate_error_d = calculate_turnover(
    a = 0.7,
    FL = 0.5,
    F0 = 0,
    FT = at2H_error_percent,
    t = 7
  )
)


instrument_error_test <- crossing(
  t = seq(1, 30, by = 0.05),
  FL = c(0.1, 0.25, 0.5, 1, 2.5, 5)
) %>% mutate(
  a = 0.7,
  F0 = 0.00000000000000000000001,
  FT = instrument_error,
...
p_IRMS_error_prop <- instrument_error_test %>% 
  ggplot(
    aes(
      x = t,
      y = u_d,
  ...
  labs(
    ...
    y = TeX("Growth Rate Uncertainty (day $^{-1})"),
    color = "Tracer Strength (%)",
    fill = "Tracer Strength (%)",
  ) +
  theme(
...
cowplot::save_plot(
  filename = "fig_output/p_IRMS_error_prop.pdf",
  plot = p_IRMS_error_prop,
  base_height = 5,
  base_width = 6
)
```
...
turnover_summarized %>% 
  ...
  labs(
    x = TeX("Growth Rate (d$^{-1}$)"),
    y = TeX("Memory Corrected (d$^{-1}$)")
  ) +
  theme_classic()
```
...
turnover_summarized %>% 
  ...
  pivot_longer(
    ...
    values_to = "u_d") %>% 
  group_by(compound) %>%
  mutate(y = paste(round(mean(rt), 0), compound)) %>% 
  ungroup() %>% 
  arrange(desc(y)) %>% 
  ggplot(
    aes(
...
error_matrix <-
  tribble(
     ...
     0,       0,      1,       0,       "F0 error only",
     0,       0,      0,       1,       "FT error only"
  ) %>%
...
model_data <- 
  tibble(
  a = 0.7069045, # assimilation factor
  ...
  # dynamics
  crossing(
    FL = c(5000, 10000, 25000, 50000), # ppm
    F2_interval = seq(-10, 0, by = 0.1) %>% exp()
  ) %>%
  # mu calculations
  mutate(
    F2 = F1 + FL * F2_interval,
    FL_factor = paste(FL/1e6 * 100, "at %") %>% as_factor(),
  ...
  mutate(
    sigma_mu.1_day = calculate_sigma_mu(
      ...
      sigma_F1 = use_F1 * sigma_F1, sigma_FL = use_FL * sigma_FL
    ),
...
# plot
p_model_data <- model_data %>%
  ggplot() + 
  aes(x = TD.days / t.days, y = rel_error, color = FL_factor, linetype = t_factor) + 
  geom_line() +
  scale_x_log10(
        breaks = trans_breaks("log10", function(x) 10^x),
        labels = trans_format("log10", math_format(10^.x)),
  ...
  facet_wrap(~panel) +
  scale_color_viridis_d(option = "plasma", begin = 0, end = 0.6) +
  theme_bw() +
  theme(
    legend.position = c(0.85, 0.2),
...
cowplot::save_plot(
  plot = p_model_data,
  filename = "fig_output/p_model_data.pdf",
  base_height = 8,
  base_width = 10
)
...
# Supplementary Dataset Export

```{r}
# SDS1: Sent from Colorado State Lab
# SDS2: Exported BacDive Data
# SDS3: Manually-curated in Excel

# SDS4:
# SDS4.1: Summarized Growth Rates
sds4_1 <- turnover_summarized %>% 
  select(terrain, inc_time_d, compound, compound_ug_per_g_soil,
         u_d_mc, su_d_mc, gen_d_mc)

# SDS4.2: Assemblage-level growth rates
sds4_2 <- turnover_weighted %>% 
  filter(inc_time_d == 7) %>% 
  select(terrain, f_label, dF_weighted_mc, 
        u_d_mc_weighted, gen_d_mc_weighted)

# SDS4.3: IRMS Data
sds4_3 <- LH_SIP_data %>% 
  select(-c(soil, inc_time_d_str, d2H_mn_zero, d2H, at2H, compound_ug_per_g_soil, 
            `2F_Me`, `2F_FAME`, `2F_FAME_mc`, `2F_alk`, `2F_alk_mc`, `F_label_mn`,
            `F_label_se`, `F_0_at2H_se`, sa, a))

# SDS4.4: Uncertainty parameters
sds4_4 <- turnover_summarized %>% 
  filter(inc_time_d == 7) %>% 
  select(terrain, compound, 
         calibrated_at2H_with_memory_correction_se, F_label_se, 
         F_0_at2H_se, sa, su_d_mc) %>% 
  rename(
    s_ft = calibrated_at2H_with_memory_correction_se,
    s_fl = F_label_se,
    s_f0 = F_0_at2H_se,
    s_a = sa,
    s_u_d_mc = su_d_mc
...
writexl::write_xlsx(
  sds4_1, 
  path = "data_output/sds4_1.xlsx"
  )

writexl::write_xlsx(
  sds4_2, 
  path = "data_output/sds4_2.xlsx"
  )

writexl::write_xlsx(
  sds4_3,
  path = "data_output/sds4_3.xlsx"
)

writexl::write_xlsx(
  sds4_4,
  path = "data_output/sds4_4.xlsx"
)

writexl::write_xlsx(
  s_u_d_mc_compound,
  path = "data_output/sds4_5a.xlsx"
)

writexl::write_xlsx(
  s_u_d_mc_terrain,
  path = "data_output/sds4_5b.xlsx"
)

writexl::write_xlsx(
  s_u_d_mc_total,
  path = "data_output/sds4_5c.xlsx"
)
```
