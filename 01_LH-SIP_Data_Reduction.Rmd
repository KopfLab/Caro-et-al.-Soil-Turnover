...
This uses memory-effect corrected data. See SI.

```{r}
LH_SIP_memcorr <- LH_SIP_memcorr %>% 
...
## Join raw and mem-corrected (mc) datasets
...
peaks_mapped_with_zeros_FID_mc_pame <- peaks_mapped_with_zeros_FID_mc %>% 
...
# for 3-7 timepoints
peaks_mapped_with_threes_FID_mc_pame <- peaks_mapped_with_threes_FID_mc %>% 
  left_join(compound_nC_nH, by = "compound") %>% 
  mutate(`2F_Me` = PAME_summarized %>% pull(at2H_mn),
         # define calibrated 2F
         `2F_FAME` = calibrated_at2H_without_memory,
...
# For 3 -7 timepoints
# Analytes
peaks_mapped_with_threes_analytes <- peaks_mapped_with_threes_FID_mc_pame %>% 
  # Don't want our standard compounds to be used for generation time calculations! x_x
  filter(str_detect(compound, "STD", negate = TRUE)) %>% 
  # Also don't want our yeast-extract stimulated condition in our analyte pool
  filter(str_detect(sample_id, "t0t", negate = TRUE)) %>% 
...
peaks_mapped_with_zeros_analytes_reduced <- peaks_mapped_with_zeros_analytes %>% 
  ...
  select(file_id,
         ...
         `2F_Me`,
         `2F_FAME`,
         `2F_FAME_mc`, # Memory corrected value
         `2F_alk`, # Memory corrected value
         `2F_alk_mc`,
         F_label_mn,
         F_label_se,
         time_course
