...
gen_calc_data <- peaks_mapped_with_zeros_analytes %>% 
  ...
  mutate(
    ...
    #u_d_se = # pull from wikipedia equation --> describe
...
### Define a function to iterate across assimilation efficiency values
```{r}
generate_assim_error_prop <- 
  function(df_init, a_list = seq(from = 0.35, to = 0.8, by = 0.05)) {
    
    gen_calc_temp <- df_init
    
    for (k in a_list) {
      gen_calc_temp <- gen_calc_temp %>% 
      mutate(a = k) %>% 
      bind_rows(gen_calc_temp)
    }
    
    return(gen_calc_temp)
  }
...
gen_calc_errorprop <- peaks_mapped_with_zeros_analytes %>% 
  # FILTER OUT NON BACTERIAL COMPOUNDS
  filter(compound %nin% c("20:0", "22:0", "24:0")) %>% 
  mutate(
    f_label = 0.5, # label is 0.5 at%
...
write_csv(gen_calc_errorprop, file = "data_output/gen_calc_errorprop.csv")
